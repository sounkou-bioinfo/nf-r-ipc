...
```{nextflow eval=TRUE, args='-plugins nf-r-ipc@0.2.1'}
...
- Utilities: `naType`, `isMissing`, `coalesce`, `coalesceNULL`, `coalesceNA`, `assertNotMissing`, `renderValue`, `asLocalDate`, `asInstantUtc`, `asZonedDateTime`, `asDurationSeconds`
...
- `NfrResponseException` for structured R error envelopes (`status=error`)
...
- Use `asLocalDate/asInstantUtc/asZonedDateTime` helpers for opt-in Java time conversion.
- Deep list-columns and arbitrary R object graphs are not a supported wire shape.
- Deep nested list/map values are supported; arbitrary R object identity/class round-trip is not guaranteed.
